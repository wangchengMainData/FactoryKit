...
package com.gosuncn.zfyfactorytest.LightSensor;
...
public class LightSensor extends BaseActivity {
	...
	private float mALSPreviousValue = -1.0f;
	@Override
	public void finish() {
...
//			getConfirmText().setText(alsData);
...
//			setContentView(loadDefaultConfirmText(""));
	...
	protected void onResume() {
		...
		if(Values.AUTO_TEST_ENABLED) {
			hideTestResultConfirmButtonBar();
		}
	}
			...
			synchronized (this) {
				if (event.sensor.getType() == SENSOR_TYPE) {
					...
					if(Values.AUTO_TEST_ENABLED || Framework.quickTestEnabled) {
						...
						if(mALSPreviousValue < 0) {
							mALSPreviousValue = event.values[0];
							mTextView.setText(getString(R.string.lightsensor_summary) + Float.toString(event.values[0]));
						}else{
							if(Math.abs(event.values[0] - mALSPreviousValue) > 50){
								mTextView.setText(getString(R.string.lightsensor_summary) + Float.toString(event.values[0]));
								mALSPreviousValue = event.values[0];
							}
						}
